...
| 3 | Live agents: pool + camera-aware sampler (the ADR-002 chokepoint), transform buffer in snapshots (transferable), renderer agent layer, pinned cims | sim+protocol+renderer | GDD §9.4, TDD §6.5/§7 | XL | sampler-chokepoint unit; follow test e2e (exit criterion 1) | in-progress (protocol v8 #51; sim #52: pool + camera-aware sampler on the canonical twin, unhashed visual rng, PROJECTION-PURITY test, canonical pins + viewportHint v9; renderer layer + follow e2e next) |
