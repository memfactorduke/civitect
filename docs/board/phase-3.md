...
| 5 | Jam diagnosis: under-built bridge scenario → advisor with cause chain pointing at the saturated edge (exit criterion 4) | sim+e2e | GDD §9, ADR-009 | M | sim-level: under-built corridor saturates → alert advisor whose edge ref RESOLVES (alive + over capacity); browser e2e rides the overlay/inspector tranche | in-review |
