...
**STATUS: ALL THREE EXIT CRITERIA GREEN (2026-06-12)** — (1) the
congested-street fire differential passes (#62): same geometry + same
morning-peak ignition, jammed ⇒ spread + cause.truckLate chain whose
building/edge/station links resolve to live entities, free ⇒ contained;
(2) coverage ≡ per-station network-distance ground truth as a property
(#59, 25 CI runs, held at 250 locally); (3) goldens re-blessed with
services (#64): services-city-01 blessed new, balance reference city
fully serviced, every pre-existing golden byte-identical through the
phase. Content-independent Phase 4 scope is COMPLETE. Deferred (recorded
per-row): multi-hop fire spread (Phase 8), service-vehicle volume
injection (Phase 5 freight), police/crime economics (Phase 5 land
value), district budget overrides (Phase 6), wind fields (map-gen v2),
service upkeep costs (Phase 5 budget cycle).
