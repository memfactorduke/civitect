...
| 10 | Road tools end-to-end: drag→buildRoad command→sim→snapshot→chunk redraw; bulldoze; undo/redo binding; Playwright e2e | app + e2e | TDD §1/§7 | M | e2e: command-loop behaviors through the worker (build/undo depth, bulldoze rejects); hash identity proven in sim units — full tool UX awaits road rendering | 8 | parked — stacked on 8 (PR open, green) |
| 11 | 500-segment network perf scenario on an L map: golden + render frame budget (exit criterion 1) joins the perf gate | e2e | TDD §2/§12 | M | perf gate green at 500 segments on L map (tick p95 0.0001 ms); render-frame half awaits road rendering | 8 | parked — stacked on 8 (PR open, green) |
