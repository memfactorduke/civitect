...
**As-built (Phase 6, 2026-07 — the transit / mode-choice / policy realization; the numbered spec above is the aspiration, this is what shipped):**
- *Mode choice (item 2)* is float/exp-free per ADR-005: transit competes with the CAR on the car margin via an INTEGER discrete-choice split — a scale-invariant cost-RATIO `cTransit/cCar` (permille) read from a hardcoded SHARP step S-curve (`transitShareFromRatio`), 0 beyond 1.8× so an OD with no competitive line is 100% car (keeps transit-free cities byte-identical). Transit generalized cost = fraction-of-congested-car-time (in-vehicle) + wait (½ headway) + access-walk. The MONETARY (fare) term is not yet in the cost — deferred [TODO: free-transit adds a fare term the ordinance zeroes]. Walk/bike remain the existing short-trip walk bucket; a full 4-mode logit is deferred.
- *Transit (item 6):* lines are ordered stop lists with a per-line ledger (riders/cost/fare, integer cents). Ridership is attributed to lines at solve-finalize by SERVED-DEMAND WEIGHT (largest-remainder, exact) — NOT stop-queue boarding. Economics book at the monthly close: fares → a `transitFare` report line, per-mode vehicle upkeep → `serviceUpkeep`. Vehicle CAPACITY / overcrowding is DEFERRED (task 4b) — transit is currently uncapped; the design hit a load-divergence wall (per-line ridership feeding the hashed split must equilibrate + be reproducible on load) and awaits a deliberate save-format design. `ridden` is a derived conservation bucket (generated − assigned − walked − unroutable), never hashed/saved.
- *District-aware traffic policies (new, GDD §11):* the CONGESTION CHARGE folds a free-flow-proportional integer surcharge into the twin cost field (`twinCosts`) on charged-district edges (resolved by edge-midpoint tile), so cars reroute AND the higher car cost lifts transit share; freight routes on the charge-free base. The TRUCK BAN adds a freight-only surcharge in the freight dijkstra so trucks detour. Both maps (`chargeByKey`/`banByKey`) are DERIVED (recomputed on load + on an epoch fence that re-applies them the tick a policy/paint command lands, which also re-routes freight for the ban), keyed by canonical edge id — so a mid-hour toggle is load-invariant (ADR-005; adversarial-review-hardened). All integer; a policy-free city is byte-identical (no golden re-bless). [TUNE: charge 800‰, ban 6000‰ of free-flow; the charge is milestone-gated at congestion-pricing/30k.]
