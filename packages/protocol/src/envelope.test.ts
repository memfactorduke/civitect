...
describe("message envelope", () => {
  ...
  it("pins the selectTile command wire layout (v14 stamp; body unchanged since v1)", () => {
    const bytes = encodeMessage({
      kind: MessageKind.command,
      body: { seq: 1, tick: 2, type: CommandType.selectTile, x: 3, y: 4 },
    });
    expect(toHex(bytes)).toBe(
      ["0e00", "01", "12000000", "01000000", "0200000000000000", "0100", "0300", "0400"].join(""),
    );
  });

  it("pins the saveResponse wire layout (v14 stamp; body unchanged since v2)", () => {
    const bytes = encodeMessage({
  ...
  it("pins the loadResponse wire layout (v14 stamp; body unchanged since v2)", () => {
    const bytes = encodeMessage({
  ...
  it("pins the buildRoad command wire layout (v14 stamp; body unchanged since v3)", () => {
    const bytes = encodeMessage({
      kind: MessageKind.command,
      body: {
        seq: 1,
  ...
  it("pins the snapshot wire layout (v14 stamp; agent rider contract since v8)", () => {
    const bytes = encodeMessage({
  ...
  it("pins the viewportHint wire layout (v14 stamp; body unchanged since v9)", () => {
    const bytes = encodeMessage({
  ...
  it("pins the setServiceBudget command wire layout (new in v11)", () => {
    ...
    expect(toHex(bytes)).toBe(
      [
        ...
        "0d00", // CommandType.setServiceBudget (=13; only the protocol version bumps to v14)
        "03", // ServiceId.health
        "e204", // 1250 permille
      ].join(""),
    );
